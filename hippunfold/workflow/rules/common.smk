...
def get_final_spec():
    ...
    if len(config['hemi']) == 2:
        specs = expand(
            bids(root=root,datatype='surf',den='{density}',space='{space}',suffix='hippunfold.spec', **config['subj_wildcards']),
                density=config['output_density'],
                space=surf_spaces,
                allow_missing=True)
    else:
         specs = expand(
            bids(root=root,datatype='surf',den='{density}',space='{space}',hemi='{hemi}',suffix='hippunfold.spec', **config['subj_wildcards']),
                density=config['output_density'],
                space=surf_spaces,
                hemi=config['hemi'],
                allow_missing=True)
...
def get_final_coords():
    ...
    coords.extend(
                expand(
                    bids(
                        root=root,datatype='seg',dir='{dir}',suffix='coords.nii.gz', desc='{desc}',space='{space}',hemi='{hemi}', **config['subj_wildcards']),
                            desc='laplace',
                            dir=['AP','PD','IO'],
                            hemi=config['hemi'],
                            space=config['output_spaces'],
