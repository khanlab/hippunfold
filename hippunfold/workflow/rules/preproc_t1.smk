...
if config['skip_preproc']:
    ...
    rule n4_t1:
        ...
        output:
            t1 = bids(root=root,datatype='anat',**config['subj_wildcards'],desc='preproc', suffix='T1w.nii.gz'),
        threads: 8
...
rule reg_to_template:
    input: 
        ...
        ref = os.path.join(workflow.basedir,'..',config['template_files'][config['template']]['T1w']),
...
rule qc_reg_to_template:
    ...
    output:
        png = report(bids(root=root,datatype='qc',**config['subj_wildcards'],suffix='regqc.png',from_='subject', to=config['template']),
...
rule compose_template_xfm_corobl:
    input:
        ...
        std_to_cor = os.path.join(workflow.basedir,'..',config['template_files'][config['template']]['xfm_corobl'])
    output:
