...
rule calculate_surface_area:
    input: 
        gii = bids(root=root,datatype='surf',den='{density}',suffix='midthickness.surf.gii', space='{space}',hemi='{hemi}', label='{autotop}', **config['subj_wildcards'])
    output:
        gii = bids(root=root,datatype='surf',den='{density}',suffix='surfarea.shape.gii', space='{space}',hemi='{hemi}', label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule smooth_surface:
    ...
    params:
        ...
        iterations = 100
    output:
        gii = bids(root=root,datatype='surf',den='{density}',suffix='midthickness.surf.gii', space='{space}',hemi='{hemi}', label='{autotop}', desc='smoothed', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule calculate_curvature_from_surface:
    input: 
        gii = bids(root=root,datatype='surf',den='{density}',suffix='midthickness.surf.gii', space='{space}',hemi='{hemi}', label='{autotop}', desc='smoothed',**config['subj_wildcards'])
    output:
        gii = bids(root=root,datatype='surf',den='{density}',suffix='curvature.shape.gii', space='{space}',hemi='{hemi}', label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule calculate_thickness_from_surface:
    input: 
        inner = bids(root=root,datatype='surf',den='{density}',suffix='inner.surf.gii', space='{space}',hemi='{hemi}', label='{autotop}', **config['subj_wildcards']),
        outer = bids(root=root,datatype='surf',den='{density}',suffix='outer.surf.gii', space='{space}',hemi='{hemi}', label='{autotop}', **config['subj_wildcards'])
    output:
        gii = bids(root=root,datatype='surf',den='{density}',suffix='thickness.shape.gii', space='{space}',hemi='{hemi}', label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
def get_inputs_cifti_metric(wildcards):
    files = dict()
    if 'L' in config['hemi']:
        files['left_metric'] = bids(root=root,datatype='surf',den='{density}',suffix='{metric}.shape.gii', space='{space}',hemi='L', label='{autotop}', **config['subj_wildcards']).format(**wildcards),
    if 'R' in config['hemi']:
        files['right_metric'] = bids(root=root,datatype='surf',den='{density}',suffix='{metric}.shape.gii', space='{space}',hemi='R', label='{autotop}', **config['subj_wildcards']).format(**wildcards),
    return files
...
rule create_dscalar_metric_cifti:
    ...
    output:
        cifti = bids(root=root,datatype='surf',den='{density}',suffix='{metric}.dscalar.nii', space='{space}', label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmd}'
...
def get_inputs_cifti_label(wildcards):
    files = dict()
    if 'L' in config['hemi']:
        files['left_label'] = bids(root=root,datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='L', label='hipp', **config['subj_wildcards']).format(**wildcards),
    if 'R' in config['hemi']:
        files['right_label'] = bids(root=root,datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='R', label='hipp', **config['subj_wildcards']).format(**wildcards),
    return files
...
rule create_dlabel_cifti_subfields:
    ...
    output:
        cifti = bids(root=root,datatype='surf',den='{density}',suffix='subfields.dlabel.nii', space='{space}', label='hipp', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmd}'
...
rule create_spec_file_hipp:
    input:
        shapes = expand(bids(root=root,datatype='surf',den='{density}',suffix='{shape}.shape.gii', space='{space}',hemi='{hemi}', label='hipp', **config['subj_wildcards']),
                    shape=['gyrification','curvature','thickness'], allow_missing=True),
        surfs = expand(bids(root=root,datatype='surf',den='{density}',suffix='{surfname}.surf.gii', space='{space}', hemi='{hemi}', label='hipp', **config['subj_wildcards']),
                    surfname=['midthickness','inner','outer'], space=['{space}','unfolded'], allow_missing=True), 
        subfields = bids(root=root,datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='{hemi}', label='hipp', **config['subj_wildcards']),
        cifti = expand(bids(root=root,datatype='surf',den='{density}',suffix='{cifti}.nii', space='{space}', label='hipp', **config['subj_wildcards']),
                    cifti=['gyrification.dscalar','curvature.dscalar','thickness.dscalar','subfields.dlabel'], allow_missing=True),
    params:
        cmds = get_cmd_spec_file
    output: 
        spec_file = bids(root=root,datatype='surf',den='{density}',suffix='surfaces.spec', hemi='{hemi,L|R}',space='{space}', label='hipp', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmds}'
...
rule merge_lr_spec_file:
    input:
        spec_files = expand(bids(root=root,datatype='surf',den='{density}',suffix='surfaces.spec', hemi='{hemi}',space='{space}', label='{autotop}', **config['subj_wildcards']),
