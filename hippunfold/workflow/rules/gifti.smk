...
rule warp_gii_unfoldtemplate2unfold: 
    ...
    input: 
        warp = bids(root='work',**config['subj_wildcards'],suffix='autotop/Warp_unfoldtemplate2unfold.nii',desc='cropped',space='corobl',hemi='{hemi}'),
...
rule warp_gii_unfold2native: 
    input: 
        warp = bids(root='work',**config['subj_wildcards'],suffix='autotop/Warp_unfold2native.nii',desc='cropped',space='corobl',hemi='{hemi}'),
        gii = bids(root='work',datatype='surf',den='{density}',suffix='{surfname}.surf.gii', desc='constrainbbox',space='unfolded',hemi='{hemi}', **config['subj_wildcards'])
    params:
        structure_type = lambda wildcards: hemi_to_structure[wildcards.hemi],
        secondary_type = lambda wildcards: surf_to_secondary_type[wildcards.surfname],
...
rule calculate_surface_area:
    input: 
        gii = bids(root='results',datatype='surf',den='{density}',suffix='midthickness.surf.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    output:
        gii = bids(root='results',datatype='surf',den='{density}',suffix='surfarea.shape.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule calculate_gyrification:
    ...
    input: 
        native_surfarea = bids(root='results',datatype='surf',den='{density}',suffix='surfarea.shape.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards']),
    ...
    output:
        gii = bids(root='results',datatype='surf',den='{density}',suffix='gyrification.shape.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule smooth_surface:
    ...
    params:
        ...
        iterations = 100
    output:
        gii = bids(root='results',datatype='surf',den='{density}',suffix='midthickness.surf.gii', space='{space}',hemi='{hemi}', desc='smoothed',**config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule calculate_curvature_from_surface:
    input: 
        gii = bids(root='results',datatype='surf',den='{density}',suffix='midthickness.surf.gii', space='{space}',hemi='{hemi}', desc='smoothed',**config['subj_wildcards'])
    output:
        gii = bids(root='results',datatype='surf',den='{density}',suffix='curvature.shape.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule calculate_thickness_from_surface:
    input: 
        inner = bids(root='results',datatype='surf',den='{density}',suffix='inner.surf.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards']),
        outer = bids(root='results',datatype='surf',den='{density}',suffix='outer.surf.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    output:
        gii = bids(root='results',datatype='surf',den='{density}',suffix='thickness.shape.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule metric_to_label_gii:
    ...
    output:
        label_gii = bids(root='results',datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards'])
    group: 'subj'
    container: config['singularity']['autotop']
...
def get_inputs_cifti_metric(wildcards):
    files = dict()
    if 'L' in config['hemi']:
        files['left_metric'] = bids(root='results',datatype='surf',den='{density}',suffix='{metric}.shape.gii', space='{space}',hemi='L', **config['subj_wildcards']).format(**wildcards),
    if 'R' in config['hemi']:
        files['right_metric'] = bids(root='results',datatype='surf',den='{density}',suffix='{metric}.shape.gii', space='{space}',hemi='R', **config['subj_wildcards']).format(**wildcards),
    return files
...
rule create_dscalar_metric_cifti:
    ...
    output:
        cifti = bids(root='results',datatype='surf',den='{density}',suffix='{metric}.dscalar.nii', space='{space}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmd}'
...
def get_inputs_cifti_label(wildcards):
    files = dict()
    if 'L' in config['hemi']:
        files['left_label'] = bids(root='results',datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='L', **config['subj_wildcards']).format(**wildcards),
    if 'R' in config['hemi']:
        files['right_label'] = bids(root='results',datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='R', **config['subj_wildcards']).format(**wildcards),
    return files
...
rule create_dlabel_cifti_subfields:
    ...
    output:
        cifti = bids(root='results',datatype='surf',den='{density}',suffix='subfields.dlabel.nii', space='{space}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmd}'
...
rule create_spec_file:
    input:
        shapes = expand(bids(root='results',datatype='surf',den='{density}',suffix='{shape}.shape.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards']),
                    shape=['gyrification','curvature','thickness','coords'], allow_missing=True),
        surfs = expand(bids(root='results',datatype='surf',den='{density}',suffix='{surfname}.surf.gii', space='{space}', hemi='{hemi}', **config['subj_wildcards']),
                    surfname=['midthickness','inner','outer'], space=['{space}','unfolded'], allow_missing=True), 
        subfields = bids(root='results',datatype='surf',den='{density}',suffix='subfields.label.gii', space='{space}',hemi='{hemi}', **config['subj_wildcards']),
        cifti = expand(bids(root='results',datatype='surf',den='{density}',suffix='{cifti}.nii', space='{space}', **config['subj_wildcards']),
                    cifti=['gyrification.dscalar','curvature.dscalar','thickness.dscalar','subfields.dlabel'], allow_missing=True),
 
    params:
        cmds = get_cmd_spec_file
    output: 
        spec_file = bids(root='results',datatype='surf',den='{density}',suffix='hippunfold.spec', hemi='{hemi,L|R}',space='{space}',**config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmds}'
...
rule merge_lr_spec_file:
    input:
        spec_files = expand(bids(root='results',datatype='surf',den='{density}',suffix='hippunfold.spec', hemi='{hemi}',space='{space}',**config['subj_wildcards']),
