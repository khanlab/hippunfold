...
rule warp_gii_unfoldtemplate2unfold: 
    ...
    params:
        ...
        surface_type = 'FLAT'
    output:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='unfolded', hemi='{hemi,R|Lflip}', label='{autotop}',**config['subj_wildcards'])
...
rule warp_gii_unfold2native: 
    ...
    output:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', desc='nonancorrect', space='corobl',hemi='{hemi,R|Lflip}',label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
    shell:
        'wb_command -surface-apply-warpfield {input.gii} {input.warp} {output.gii} && '
...
rule unflip_gii:
    input:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='corobl',hemi='{hemi}flip', label='{autotop}',**config['subj_wildcards'])
    params:
        structure_type = lambda wildcards: hemi_to_structure[wildcards.hemi],
        secondary_type = lambda wildcards: surf_to_secondary_type[wildcards.surfname],
        surface_type = 'ANATOMICAL'
    output:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='corobl',hemi='{hemi,L}', label='{autotop}',**config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
    shell:
...
def get_unfolded_surf_R_Lflip (wildcards):
    if wildcards.hemi == 'R':
        return  bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='unfolded',hemi='{hemi}',label='{autotop}', **config['subj_wildcards']).format(**wildcards)
    elif wildcards.hemi == 'L':
        return  bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='unfolded',hemi='{hemi}flip',label='{autotop}', **config['subj_wildcards']).format(**wildcards)


rule unflip_gii_unfolded:
    """copy unfolded from Lflip to L"""
    input:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='unfolded',hemi='{hemi}flip',label='{autotop}', **config['subj_wildcards'])
    output:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='unfolded',hemi='{hemi,L}',label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
    shell:
...
rule warp_gii_to_T1w:
    input:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='corobl',hemi='{hemi}',label='{autotop}', **config['subj_wildcards']),
        xfm = bids(root='work',datatype='anat',**config['subj_wildcards'],suffix='xfm.txt',from_='T1w',to='corobl',desc='affine',type_='ras'),
    output:
        gii = bids(root='work',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='T1w',hemi='{hemi}',label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
    shell:
...
rule calculate_surface_area:
    ...
    output:
        gii = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='surfarea.shape.gii', space='{space}',hemi='{hemi}',label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
rule calculate_thickness_from_surface:
    input: 
        inner = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='inner.surf.gii', space='{space}',hemi='{hemi}',label='{autotop}', **config['subj_wildcards']),
        outer = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='outer.surf.gii', space='{space}',hemi='{hemi}',label='{autotop}', **config['subj_wildcards'])
    output:
        gii = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='thickness.shape.gii', space='{space}',hemi='{hemi}',label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell:
...
def get_inputs_cifti_metric(wildcards):
    files = dict()
    if 'L' in config['hemi']:
        files['left_metric'] = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='{metric}.shape.gii', space='{space}',hemi='L',label='{autotop}',  **config['subj_wildcards']).format(**wildcards),
    if 'R' in config['hemi']:
        files['right_metric'] = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='{metric}.shape.gii', space='{space}',hemi='R',label='{autotop}',  **config['subj_wildcards']).format(**wildcards),
    return files
...
rule create_dscalar_metric_cifti:
    ...
    output:
        cifti = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='{metric}.dscalar.nii', space='{space}',label='{autotop}',  **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmd}'
...
rule create_spec_file_hipp:
    input:
        shapes = expand(bids(root='results',datatype='surf_{modality}',den='{density}',suffix='{shape}.shape.gii', space='{space}',hemi='{hemi}',label='autotopHipp',  **config['subj_wildcards']),
                    shape=['gyrification','curvature','thickness'], allow_missing=True),
        surfs = expand(bids(root='results',datatype='surf_{modality}',den='{density}',suffix='{surfname}.surf.gii', space='{space}', hemi='{hemi}',label='autotopHipp',  **config['subj_wildcards']),
    ...
    output: 
        spec_file = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='hippunfold.spec', hemi='{hemi,L|R}',space='{space}',label='autotopHipp', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmds}'
...
rule create_spec_file_dentate:
    input:
        shapes = expand(bids(root='results',datatype='surf_{modality}',den='{density}',suffix='{shape}.shape.gii', space='{space}',hemi='{hemi}',label='autotopDG',  **config['subj_wildcards']),
                    shape=['gyrification','curvature','thickness'], allow_missing=True),
    ...
    params:
        cmds = get_cmd_spec_file
    output: 
        spec_file = bids(root='results',datatype='surf_{modality}',den='{density}',suffix='hippunfold.spec', hemi='{hemi,L|R}',space='{space}',label='autotopDG', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj' 
    shell: '{params.cmds}'
...
rule merge_lr_spec_file:
    input:
        spec_files = expand(bids(root='results',datatype='surf_{modality}',den='{density}',suffix='hippunfold.spec', hemi='{hemi}',space='{space}',label='{autotop}',**config['subj_wildcards']),
                        hemi=['L','R'], allow_missing=True)
    output:
        spec_file = bids(root='work',datatype='surf_{modality}',den='{density}',space='{space}',suffix='hippunfold.spec',label='{autotop}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
