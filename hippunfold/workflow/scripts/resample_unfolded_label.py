...
with open(snakemake.log[0], "w") as sys.stdout:
    ...
    arr_label = (
        label_gii.get_arrays_from_intent("NIFTI_INTENT_LABEL")[0]
        .data.reshape(n_pd, n_ap)
        .transpose()
    )
    ...
    orig_x = np.linspace(
        start_ap, end_ap, n_ap
    )  # since the original flat space was offset a bit from 0-40,0-20
