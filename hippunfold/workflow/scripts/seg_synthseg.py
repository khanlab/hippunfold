...
class ConvBlockBase(nn.Sequential):
    ...
    def __init__(self, ndim, in_channels, out_channels, opt_conv=None,
                 activation='LeakyReLU', norm=None, dropout=False, order='cand'):
        super().__init__()
...
class ConvBlock(ConvBlockBase):
    def __init__(self, ndim, in_channels, out_channels, kernel_size=3, bias=True,
                 activation='LeakyReLU', norm='instance', dropout=0, order='cand'):
        super().__init__(ndim, in_channels, out_channels,
                         opt_conv=dict(kernel_size=kernel_size, bias=bias, padding='same'),
                         activation=activation, norm=norm, dropout=dropout, order=order)
...
class ConvGroup(nn.Sequential):
    ...
    def __init__(self, ndim, in_channels, mid_channels=None, out_channels=None,
                 kernel_size=3, nb_conv=2, residual=False, activation='LeakyReLU',
                 norm='instance', dropout=0, order='cand'):
        self.residual = residual
        mid_channels = mid_channels or in_channels
        out_channels = out_channels or mid_channels
        nb_conv = nb_conv - (in_channels != mid_channels) - (out_channels != mid_channels)
        layers = []
        if in_channels != mid_channels:
            layers.append(ConvBlock(ndim, in_channels, mid_channels, kernel_size,
                                    activation=activation, norm=norm, dropout=dropout, order=order))
        for _ in range(nb_conv):
            layers.append(ConvBlock(ndim, mid_channels, mid_channels, kernel_size,
                                    activation=activation, norm=norm, dropout=dropout, order=order))
        if out_channels != mid_channels:
            layers.append(ConvBlock(ndim, mid_channels, out_channels, kernel_size,
                                    activation=activation, norm=norm, dropout=dropout, order=order))
...
class Downsample(nn.Module):
    ...
    def __init__(self, factor=2, anchor='center'):
        super().__init__(); self.factor=factor; self.anchor=anchor
...
class ConvBlockDown(nn.Sequential):
    ...
    def __init__(self, ndim, in_channels, out_channels, factor=2, kernel_size=3,
                 activation='LeakyReLU', norm='instance', dropout=0, order='cand'):
        super().__init__()
        self.downsample = Downsample(factor=factor)
        self.conv = ConvBlock(ndim, in_channels, out_channels, kernel_size=kernel_size,
                              activation=activation, norm=norm, dropout=dropout, order=order)
    def forward(self, x):
...
class ConvBlockUp(nn.Module):
    ...
    def __init__(self, ndim, in_channels, out_channels, factor=2, kernel_size=3,
                 activation='LeakyReLU', norm='instance', dropout=0, order='cand', combine='cat'):
        super().__init__()
        self.conv = ConvBlock(ndim, in_channels, out_channels, kernel_size=kernel_size,
                              activation=activation, norm=norm, dropout=dropout, order=order)
...
class EncoderBlock(nn.Sequential):
    def __init__(self, down, conv):
        super().__init__(); self.down=down; self.conv=conv
    def forward(self, x):
        return self.conv(self.down(x))

class DecoderBlock(nn.Sequential):
    def __init__(self, conv, up):
        super().__init__(); self.conv=conv; self.up=up
    def forward(self, x, skip=None):
        x = self.conv(x)
...
class UNet(nn.Module):
    ...
    def _conv_block(self,i,o=None):
        return ConvGroup(self.ndim, i, o or i, kernel_size=self.kernel_size, nb_conv=self.nb_conv,
                         activation=self.activation, norm=self.norm, dropout=self.dropout,
                         order=self.order, residual=self.residual)
    def _down_block(self,i,o=None):
        return ConvBlockDown(self.ndim, i, o or i, factor=self.factor, kernel_size=1,
                             activation=self.activation, norm=self.norm, dropout=self.dropout, order=self.order)
    def _up_block(self,i,o=None):
        return ConvBlockUp(self.ndim, i, o or i, factor=self.factor, kernel_size=1,
                           activation=self.activation, norm=self.norm, dropout=self.dropout, order=self.order, combine=self.combine)
    def forward(self, x):
        nb_levels = len(self.encoder)
        if any(s < 2**nb_levels for s in x.shape[2:]):
            raise ValueError(f'UNet with {nb_levels} levels requires input spatial dims >= {2**nb_levels}')
        skips = []
...
class SegNet(nn.Sequential):
    def __init__(self, ndim, in_channels, out_channels, kernel_size=3, activation='Softmax',
                 backbone='UNet', kwargs_backbone=None):
        if isinstance(backbone,str):
            backbone_cls=globals()[backbone]
            backbone=backbone_cls(ndim, **(kwargs_backbone or {}))
        act=None
        if activation and isinstance(activation,str) and activation.lower()=='softmax':
            act=nn.Softmax(1)
        feat=ConvBlock(ndim,in_channels, backbone.in_channels, kernel_size=kernel_size, activation=None, norm=None)
        pred=ConvBlock(ndim, backbone.out_channels, out_channels, kernel_size=1, activation=act, norm=None)
        super().__init__(feat, backbone, pred)

class SynthSeg(nn.Module):
    def __init__(self, segnet):
        super().__init__(); self.segnet=segnet
    def forward(self,x): return self.segnet(x)
...
class Model(nn.Module):
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        backbone = UNet(ndim, nb_levels=seg_nb_levels, nb_features=seg_features,
                        nb_conv=seg_nb_conv, activation=seg_activation, norm=seg_norm)
...
class Real(torch.nn.Module):
    def __init__(self, synth): super().__init__(); self.synth = synth
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SynthSeg hippocampus segmentation inference')
    ...
    parser.add_argument('-o', '--output', required=True, help='Output segmentation file')
