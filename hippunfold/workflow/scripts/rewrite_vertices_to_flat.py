...
vertices[:, 0] = ap * snakemake.params.vertspace["extent"][0].astype(
    float32
)  # + snakemake.params.vertspace["origin"][0]
vertices[:, 1] = pd * snakemake.params.vertspace["extent"][1].astype(
    float32
)  # + snakemake.params.vertspace["origin"][1]
vertices[:, 2] = snakemake.params.z_level.astype(float32)
