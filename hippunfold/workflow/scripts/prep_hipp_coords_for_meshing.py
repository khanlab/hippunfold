...
hl = np.ones([3, 3, 3])
hl = hl / np.sum(hl)
convolved_coords = convolve(
    coords_withnan, hl, nan_treatment="interpolate", preserve_nan=False
)

# dilating the mask into the src and sink
structuring_element = np.ones((3, 3, 3), dtype=bool)
dilated_mask = binary_dilation(gm_noDG_mask, structuring_element) & (
    src_mask | sink_mask
)
updated_mask = np.where(dilated_mask, 1, gm_noDG_mask)

# get the updated coords:
updated_coords = np.where(gm_mask, coords, np.where(dilated_mask, convolved_coords, 0))


# save the extrapolated coords and the updated mask
nib.Nifti1Image(convolved_coords, coords_nib.affine, coords_nib.header).to_filename(
    snakemake.output.coords
)
nib.Nifti1Image(updated_mask, coords_nib.affine, coords_nib.header).to_filename(
