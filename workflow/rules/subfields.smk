...
rule get_subfield_vols_subj:
    ...
    input:
        ...
        lookup_tsv=os.path.join(
            config["snakemake_dir"], "resources", "desc-subfields_dseg.tsv"
        ),
    group:
        "subj"
    output:
...
rule concat_subj_vols_tsv:
    ...
    input:
        tsv=lambda wildcards: expand(
            bids(
                root=root,
                datatype=f"seg_{wildcards.modality}",
                desc="subfields",
                suffix="volumes.tsv",
                **config["subj_wildcards"],
            ),
            subject=config["input_lists"][get_modality_key(wildcards.modality)][
                "subject"
            ],
