...
rule label_subfields_from_vol_coords_corobl:
    ...
    input:  
        ...
        nii_ap = bids(root=work,datatype='seg_{modality}',dir='AP',suffix='coords.nii.gz', desc='laplace',space='corobl',hemi='{hemi}', **config['subj_wildcards']),
        nii_pd = bids(root=work,datatype='seg_{modality}',dir='PD',suffix='coords.nii.gz', desc='laplace',space='corobl',hemi='{hemi}', **config['subj_wildcards'])
    params:
...
rule resample_subfields_to_T1w:
    ...
    output:
        nii = bids(root=root,datatype='seg_{modality}',suffix='dseg.nii.gz', desc='subfields',space='T1w',hemi='{hemi}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
    shell:
...
rule get_subfield_vols_subj:
    ...
    input: 
        segs = expand(bids(root=root,**config['subj_wildcards'],datatype='seg_{modality}',hemi='{hemi}',space='cropT1w',desc='subfields',suffix='dseg.nii.gz'),
                    hemi=config['hemi'], allow_missing=True),
        lookup_tsv = os.path.join(config['snakemake_dir'],'resources','desc-subfields_dseg.tsv')
    group: 'subj'
    output: 
        tsv = bids(root=root,datatype='seg_{modality}',desc='subfields',suffix='volumes.tsv',**config['subj_wildcards']),
...
rule plot_subj_subfields:
    input:
        tsv = bids(root=root,datatype='seg_{modality}',desc='subfields',suffix='volumes.tsv',**config['subj_wildcards'])
    output:
        png = report(bids(root=root,datatype='qc',desc='subfields',from_='{modality}',suffix='volumes.png',**config['subj_wildcards']),
...
def get_bg_img_for_subfield_qc(wildcards):
    ...
    return  bids(root=root,datatype='seg_{modality}',desc='preproc',suffix=f'{bg_modality}.nii.gz', space='cropT1w',hemi='{hemi}', **config['subj_wildcards'])

rule qc_subfield:
    input:
        img = get_bg_img_for_subfield_qc,
        seg = bids(root=root,datatype='seg_{modality}',suffix='dseg.nii.gz', desc='subfields',space='cropT1w',hemi='{hemi}', **config['subj_wildcards']),
    output:
        png1 = report(bids(root=root,datatype='qc',suffix='dseg.png', desc='subfields',from_='{modality}',slice_='1',space='cropT1w',hemi='{hemi}', **config['subj_wildcards']),
                caption='../report/subfield_qc.rst',
                category='Segmentation QC',
                subcategory='Subfields from {modality}'),
        png2 = report(bids(root=root,datatype='qc',suffix='dseg.png', desc='subfields',from_='{modality}',slice_='2',space='cropT1w',hemi='{hemi}', **config['subj_wildcards']),
                caption='../report/subfield_qc.rst',
                category='Segmentation QC',
                subcategory='Subfields from {modality}'),
        png3 = report(bids(root=root,datatype='qc',suffix='dseg.png', desc='subfields',from_='{modality}',slice_='3',space='cropT1w',hemi='{hemi}', **config['subj_wildcards']),
                caption='../report/subfield_qc.rst',
                category='Segmentation QC',
                subcategory='Subfields from {modality}'),
    group: 'subj'
...
rule qc_subfield_surf:
    input:
    	surf = bids(root=root,datatype='surf_{modality}',suffix='midthickness.surf.gii',space='T1w',hemi='{hemi}', **config['subj_wildcards']),
    output:
        png = report(bids(root=root,datatype='qc',suffix='midthickness.surf.png', desc='subfields',from_='{modality}',space='cropT1w',hemi='{hemi}', **config['subj_wildcards']),
                caption='../report/subfield_qc.rst',
                category='Segmentation QC',
                subcategory='Subfields from {modality}'),
    group: 'subj'
...
rule concat_subj_vols_tsv:
    ...
    input: 
        tsv = lambda wildcards: expand(bids(root=root,datatype=f'seg_{wildcards.modality}',desc='subfields',suffix='volumes.tsv',**config['subj_wildcards']),
                    subject=config['input_lists'][get_modality_key(wildcards.modality)]['subject'],
                    session=config['sessions'])
    group: 'aggregate'
    output: 
        tsv = bids(root=root,prefix='group',from_='{modality}',desc='subfields',suffix='volumes.tsv'),
    run: 
        import pandas as pd
...
rule resample_subfields_to_unfold:
    ...
    output:
        nii = bids(root=root,datatype='seg_{modality}',suffix='dseg.nii.gz', desc='subfields',space='unfold',hemi='{hemi}', **config['subj_wildcards'])
    container: config['singularity']['autotop']
    group: 'subj'
    shell:
        'ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS={threads} '
