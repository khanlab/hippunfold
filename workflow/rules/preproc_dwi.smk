...
rule mrdegibbs:
    ...
    container:
        config["singularity"]["prepdwi"]
    log:
        bids(root="logs", suffix="degibbs.log", **config["input_wildcards"]["dwi"]),
    group:
        "subj"
    shell:
...
rule get_phase_encode_txt:
    ...
    group:
        "subj"
    script:
        "../scripts/preproc_dwi/get_phase_encode_txt.py"
...
rule get_eddy_index_txt:
    ...
    group:
        "subj"
    script:
        "../scripts/preproc_dwi/get_eddy_index_txt.py"
...
rule concat_runs_bvec:
    ...
    group:
        "subj"
    script:
        "../scripts/preproc_dwi/concat_bv.py"
...
rule get_slspec_txt:
    ...
    group:
        "subj"
    script:
        "../scripts/preproc_dwi/get_slspec_txt.py"
...
rule run_eddy:
    ...
    params:
        ...
        options=" ".join(
            [
                f"--{key}={value}"
                for (key, value) in config["eddy"]["opts"].items()
    ...
    output:
        ...
        out_folder=directory(
            bids(root=work, suffix="eddy", datatype="dwi", **config["subj_wildcards"])
        ),
        dwi=os.path.join(
            bids(root=work, suffix="eddy", datatype="dwi", **config["subj_wildcards"]),
            "dwi.nii.gz",
    ...
    group:
        "subj"
    shell:
        "singularity exec --nv -e {params.container} eddy_cuda9.1 --imain={input.dwi_concat} --mask={input.brainmask} "
...
rule eddy_quad:
    ...
    output:
        out_dir=directory(
            bids(
                root=work, suffix="eddy.qc", datatype="dwi", **config["subj_wildcards"]
            )
        ),
        eddy_qc_pdf=bids(
            root=work,
